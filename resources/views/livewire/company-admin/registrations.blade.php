<div class="p-4 max-w-7xl mx-auto space-y-4 relative">
    ...
    <div class="relative">
        ...
        @if($search)
            <button wire:click="$set('search', '')" class="absolute right-4 top-1/2 transform -translate-y-1/2 p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full">
    ...
    <!-- Filters -->
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md dark:shadow-gray-900/50 p-4 space-y-3">
        <div class="flex items-center justify-between">
            <h3 class="font-semibold text-gray-800 dark:text-gray-100">Filters</h3>
            @if($agentFilter || $constituencyFilter || $dateFrom || $dateTo)
                <button wire:click="clearFilters" class="text-sm text-red-600 dark:text-red-400 font-medium hover:text-red-700 dark:hover:text-red-300">
                    Clear All
                </button>
            @endif
        </div>
        ...
        <!-- Agent Filter -->
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Agent</label>
            <select
            ...
            >
                ...
                @foreach($agents as $agent)
                    <option value="{{ $agent->id }}">{{ $agent->name }} ({{ $agent->registeredMembers->count() }})</option>
                @endforeach
            </select>
        </div>
    ...
    @if($search || $agentFilter || $constituencyFilter || $dateFrom || $dateTo)
                ...
                <table class="w-full">
                    ...
                    <thead class="bg-gradient-to-r from-uda-yellow-500 to-uda-green-500 text-white">
                        <tr>
                            ...
                            <th class="px-4 py-4 text-left text-sm font-bold uppercase tracking-wider">Registered By</th>
    ...
    @else
        <!-- Empty State -->
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md dark:shadow-gray-900/50 p-12 text-center">
            ...
            <button wire:click="clearFilters" class="px-6 py-3 bg-uda-green-500 text-white font-semibold rounded-lg hover:bg-uda-green-600 transition">
                Clear Filters
            </button>
        </div>
    @endif
